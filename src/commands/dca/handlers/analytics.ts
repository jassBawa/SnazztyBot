...
import { getUserByTelegramId } from "../../../services/db";
import { getDcaStrategy, getUserDcaStrategies } from "../../../services/db/dca/strategy";
import {
  getStrategyExecutions,
  getUserExecutions,
  getStrategyWithExecutions,
} from "../../../services/db/dca/execution";
...
export function registerAnalyticsHandlers(bot: Telegraf) {
  ...
  bot.action(/^DCA_CANCEL_OK_(.+)$/, async (ctx) => {
    try {
      ...
      const { cancelDcaStrategy } = await import("../../../services/db/dca/strategy");
  ...
  bot.action(/^DCA_RESTART_(.+)$/, async (ctx) => {
    try {
      ...
      const { resumeDcaStrategy } = await import("../../../services/db/dca/strategy");
